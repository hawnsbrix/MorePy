...
def make_map():
	...
	for r in range(MAX_ROOMS):
		...
		y = libtcod.random_get_int(0, 0, MAP_HEIGHT - h - 2) #if it helps with the edge of the map
	...
	stairs = Object(new_x, new_y, 'H', 'stairs', libtcod.white, always_visible=True)
	...
	for i in range(num_monsters):
		...
		if not is_blocked(x, y):
			...
			if choice == 'aardwurm':
				...
				monster = Object(x, y, 'G', 'Aardwurm', libtcod.gold,
				...
				monster = Object(x, y, 'S', 'Fiend', libtcod.orange,
					blocks=True, fighter=fighter_component, ai=ai_component)
...
def previous_level():
	...
	message('There is no way back for now!', libtcod.light_red)
	##dungeon_level -= 1
...
def render_all():
	...
	global char_for_light_walls, char_for_dark_walls
	global char_color_on_light_walls, char_color_on_dark_walls
	...
	char_color_on_light_walls = libtcod.gold
